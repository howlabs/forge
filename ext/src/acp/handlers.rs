...
pub async fn handle_acp_request(server: &mut AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    match request.method.as_str() {
        ...
        _ => Ok(error_response(&request.id, AcpError::method_not_found(&request.method))),
    }
}

fn error_response(id: &str, error: AcpError) -> AcpResponse {
    AcpResponse { id: id.into(), result: None, error: Some(error) }
}

fn success_response(id: &str, result: serde_json::Value) -> AcpResponse {
    AcpResponse { id: id.into(), result: Some(result), error: None }
}
...
fn handle_initialize(server: &mut AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    ...
    Ok(success_response(&request.id, serde_json::to_value(server.agent_info().clone())?))
}
...
async fn handle_chat(server: &AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    let params: ChatParams = match request.params.as_ref() {
        ...
        None => return Ok(error_response(&request.id, AcpError::invalid_params("Missing params"))),
    };

    match server.chat_with_model(&params.messages).await {
        Ok(content) => Ok(success_response(&request.id, serde_json::json!({
            "role": "assistant",
            "content": content
        }))),
        Err(e) => Ok(error_response(&request.id, AcpError::internal_error(&e.to_string()))),
    }
}
...
fn handle_edit(server: &AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    ...
    if let (Some(old), Some(new)) = (&params.old_text, &params.new_text) {
        match server.apply_edit(&params.file_path, old, new) {
            Ok(true) => Ok(success_response(&request.id, serde_json::json!({
                "applied": true,
                "file_path": params.file_path
            }))),
            Ok(false) => Ok(error_response(&request.id, AcpError::invalid_params("Old text not found"))),
            Err(e) => Ok(error_response(&request.id, AcpError::internal_error(&e.to_string()))),
        }
    } else {
        Ok(error_response(&request.id, AcpError::invalid_params("Missing oldText/newText")))
    }
}
...
async fn handle_command(server: &AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    ...
    match server.run_command(&params.command, &params.args).await {
        Ok(output) => Ok(success_response(&request.id, serde_json::json!({
...
fn handle_get_code_actions(_server: &AcpServer, request: &AcpRequest) -> Result<AcpResponse> {
    Ok(success_response(&request.id, serde_json::json!({ "actions": [] })))
}

fn handle_cancel(request: &AcpRequest) -> Result<AcpResponse> {
    Ok(success_response(&request.id, serde_json::json!({ "cancelled": true })))
}
...
mod tests {
    ...
    async fn test_handle_unknown_method() {
        let mut server = AcpServer::new("forge", "0.100.0");
        let req = AcpRequest { id: "1".into(), method: "unknown".into(), params: None };
        let resp = handle_acp_request(&mut server, &req).await.unwrap();
